...
public class LibraryService {
    ...
    public SinglyLinkedList<Book> getAllBooks() {
        SinglyLinkedList<Book> activos = new SinglyLinkedList<>();
        Node<Book> current = books.getHead();
        while (current != null) {
            if (current.data.isActive()) activos.add(current.data);
    ...
    public SinglyLinkedList<User> getAllUsers() {
        SinglyLinkedList<User> list = new SinglyLinkedList<>();
    ...
    public String createLoan(int userId, int bookId) {
        ...
        if (user == null || book == null) return "Usuario o libro no encontrado";
        if (!book.isActive()) return "El libro está desactivado";
